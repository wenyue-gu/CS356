...
void *sr_rip_timeout(void *sr_ptr) {
    ...
    while (1) {
        ...
        while (pointer1 != NULL) {
            ...
            if(difftime(time(0), pointer1->updated_time) >= 20 || sr_obtain_interface_status(sr,pointer1->interface)==0){
        ...
        while(interface!=NULL){
            ...
            while (pointer3 != NULL) {
                ...
                if((pointer3->dest.s_addr & pointer3->mask.s_addr) == (interface->ip & interface->mask) && pointer3->mask.s_addr == interface->mask && sr_obtain_interface_status(sr,interface->name)!=0){
